...
func HandlerMove(gs *GameState) func( ArmyMove) pubsub.AckType{
  return func(move ArmyMove) pubsub.AckType{
    defer fmt.Print(">")
    moveOutcome := gs.HandleMove(move)
    if moveOutcome == MoveOutComeSafe || moveOutcome == MoveOutcomeMakeWar {
      return pubsub.Ack
    } 
